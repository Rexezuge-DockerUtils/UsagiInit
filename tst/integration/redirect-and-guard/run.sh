...
cd "tst/integration/redirect-and-guard"
echo "=== RUNNING INTEGRATION TEST CASE: redirect-and-guard ==="

PROGRAM_PATH="../../.."
PROGRAM="$PROGRAM_PATH/UsagiInit"

# Rebuild UsagiInit with restart disabled (we only need to verify registration)
echo "=== Rebuilding UsagiInit ==="
(
    cd "$PROGRAM_PATH"
    ./scripts/build.sh -DRESTART_TERMINATED_SERVICES=OFF \
                       -DREINITIALIZE_ON_ALL_SERVICE_TERMINATION=OFF
)
